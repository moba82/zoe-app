...
const CONFETTI_COLORS=[C.mint,C.pink,C.yellow,C.pinkDark,C.mintDark,"#FFD700","#FF6B9D","#A8E6CF","#FF8B94","#B8F0E6"];
    ...
    <div style={{position:"relative",width:80}}>
      {level>60&&<div style={{position:"absolute",inset:0,pointerEvents:"none"}}>
        {[{x:-8,y:8,s:14},{x:72,y:20,s:12},{x:64,y:72,s:10},{x:-4,y:60,s:11}].map((sp,i)=>(
          <span key={i} style={{position:"absolute",left:sp.x,top:sp.y,fontSize:sp.s,animation:`sparkle 1.5s ease-in-out ${i*0.3}s infinite alternate`}}>✨</span>
        ))}
      </div>}
      ...
      <svg width={80} height={140} viewBox="0 0 80 140">
        ...
        <rect x={0} y={fillY} width={80} height={fillH+35} fill={C.mint} clipPath="url(#tc)" style={{transition:"y 0.7s cubic-bezier(.34,1.56,.64,1)"}}/>
        ...
        {happy?<path d="M30,86 Q40,96 50,86" fill="none" stroke={C.textDark} strokeWidth={2.5} strokeLinecap="round"/>
          :med?<path d="M32,87 Q40,93 48,87" fill="none" stroke={C.textDark} strokeWidth={2} strokeLinecap="round"/>
...
function ConfettiBlast({active}){
  const pieces=useRef(Array.from({length:45},(_,i)=>({
    x:15+Math.random()*70,color:CONFETTI_COLORS[i%CONFETTI_COLORS.length],
    shape:SHAPES[Math.floor(Math.random()*SHAPES.length)],size:7+Math.random()*7,
  ...
  return(
    <div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:300}}>
      <style>{`
        @keyframes burst{0%{transform:translate(0,0) rotate(0deg) scale(1);opacity:1}60%{opacity:1}100%{transform:translate(var(--vx),var(--vy)) rotate(720deg) scale(0.3);opacity:0}}
        @keyframes celebText{0%{transform:scale(0);opacity:0}25%{transform:scale(1.2);opacity:1}70%{transform:scale(1);opacity:1}100%{transform:scale(0.8) translateY(-30px);opacity:0}}
      `}</style>
      {pieces.map((p,i)=>(
      ...
      <div style={{position:"fixed",top:"28%",left:0,right:0,textAlign:"center",fontSize:32,fontWeight:900,
...
function Toggle({value,onChange}){
  return(
    <div onClick={()=>onChange(!value)} style={{width:52,height:28,borderRadius:14,
      background:value?C.mintDark:"#CCC",position:"relative",cursor:"pointer",transition:"background 0.3s"}}>
      <div style={{position:"absolute",top:3,left:value?26:3,width:22,height:22,borderRadius:"50%",
        background:"white",transition:"left 0.3s",boxShadow:"0 2px 6px rgba(0,0,0,0.2)"}}/>
    </div>
  );
}
...
export default function ZoeApp(){
  ...
  useEffect(()=>{
    fetch(`/api/calendar?day=${day}`).then(r=>r.json()).then(setCalEvents).catch(()=>{});
  },[day]);
  ...
  const completeTask=(id)=>{
    ...
    const updated=tasks.map(t=>t.id===id
      ?{...t,done:true,doneTime:new Date().toLocaleTimeString("it-IT",{hour:"2-digit",minute:"2-digit"})}:t);
    saveTasks(updated);
    setConfetti(true);
  ...
  return(
    <div style={{fontFamily:"'Nunito',sans-serif",background:C.mintBg,minHeight:"100vh",maxWidth:430,margin:"0 auto"}}>
      ...
      {/* HEADER */}
      <div style={{background:`linear-gradient(135deg,${C.mint} 0%,${C.pink} 100%)`,padding:"18px 18px 0",borderRadius:"0 0 28px 28px",boxShadow:"0 4px 20px rgba(168,223,196,0.4)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>setView("zoe")} style={{padding:"8px 18px",borderRadius:20,border:"none",
              ...
              boxShadow:view==="zoe"?"0 3px 10px rgba(0,0,0,0.12)":"none",transition:"all 0.25s"}}>🐴 Zoe</button>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <button onClick={()=>setView(view==="mom"?"zoe":"mom")} style={{background:"transparent",border:"none",
              cursor:"pointer",fontSize:18,opacity:0.35,padding:"4px 6px",lineHeight:1,color:"white"}}>♡</button>
        ...
        {view==="zoe"&&(
          <div style={{display:"flex"}}>
            {[["oggi","📅 Oggi"],["domani","🌙 Domani"]].map(([d,label])=>(
              <button key={d} onClick={()=>setDay(d)} style={{flex:1,padding:"12px 0",border:"none",cursor:"pointer",
                background:day===d?C.white:"transparent",borderRadius:day===d?"16px 16px 0 0":0,
                color:day===d?C.mintDeep:"rgba(255,255,255,0.85)",fontWeight:900,fontSize:15,
                fontFamily:"'Nunito',sans-serif",transition:"all 0.25s"}}>{label}</button>
            ))}
          </div>
        )}
      ...
      <div style={{padding:16}}>
        {view==="zoe"&&(<>
          {/* COACH */}
          <div style={{background:C.white,borderRadius:24,padding:"16px 20px",marginBottom:14,
            display:"flex",alignItems:"flex-start",gap:14,
            boxShadow:"0 4px 24px rgba(91,188,138,0.13)",border:`2px solid ${C.mintLight}`}}>
            <div style={{flexShrink:0,marginTop:4}}><CoachAvatar mood={capyMood}/></div>
            <div style={{flex:1,position:"relative"}}>
              <div style={{background:C.mintBg,borderRadius:"4px 18px 18px 18px",padding:"12px 14px",border:`1.5px solid ${C.mintLight}`}}>
                ...
                <div style={{marginTop:8,maxHeight:160,overflowY:"auto",display:"flex",flexDirection:"column",gap:6}}>
                  {chatMessages.map((m,i)=>(
                    <div key={i} style={{alignSelf:m.role==="user"?"flex-end":"flex-start",
                      ...
                      padding:"7px 12px",borderRadius:14,fontSize:13,maxWidth:"85%",
              ...
              <div style={{display:"flex",gap:6,marginTop:8}}>
                <input value={chatInput} onChange={e=>setChatInput(e.target.value)}
                  ...
                  style={{flex:1,padding:"8px 12px",borderRadius:12,border:`1.5px solid ${C.mintLight}`,
                    outline:"none",fontFamily:"'Nunito',sans-serif",fontSize:13}}/>
                <button onClick={sendChat} disabled={chatLoading} style={{padding:"8px 14px",borderRadius:12,
                  border:"none",background:C.mintDark,color:"white",fontWeight:900,cursor:"pointer",fontSize:16}}>→</button>
              </div>
              <div style={{position:"absolute",left:-10,top:14,width:0,height:0,
                borderTop:"8px solid transparent",borderBottom:"8px solid transparent",borderRight:`10px solid ${C.mintLight}`}}/>
              <div style={{position:"absolute",left:-8,top:16,width:0,height:0,
                borderTop:"6px solid transparent",borderBottom:"6px solid transparent",borderRight:`8px solid ${C.mintBg}`}}/>
            </div>
          </div>

          {/* BUBBLE TEA */}
          <div style={{background:C.white,borderRadius:24,padding:"16px 20px",display:"flex",alignItems:"center",
            gap:16,marginBottom:14,boxShadow:"0 4px 24px rgba(91,188,138,0.13)"}}>
            <KawaiiTea level={level}/>
            <div style={{flex:1}}>
              <div style={{fontWeight:900,fontSize:24,color:C.textDark}}>{done} <span style={{fontSize:16,color:C.textLight}}>/ {total}</span></div>
              <div style={{color:C.textMed,fontSize:13,marginBottom:10}}>{level===100?"🏆 Ricompensa sbloccata!":"compiti completati"}</div>
              <div style={{background:C.mintLight,borderRadius:8,height:12,overflow:"hidden"}}>
                <div style={{background:`linear-gradient(90deg,${C.mintDark},${C.pinkDark})`,width:`${level}%`,height:"100%",transition:"width 0.5s ease",borderRadius:8}}/>
              </div>
          ...
          {calEvents.length>0&&(
            <div style={{marginBottom:14}}>
              <div style={{fontSize:11,fontWeight:800,color:C.textLight,textTransform:"uppercase",letterSpacing:1.2,marginBottom:8}}>Oggi nel calendario</div>
          ...
          {/* TASKS */}
          <div style={{fontSize:11,fontWeight:800,color:C.textLight,textTransform:"uppercase",letterSpacing:1.2,marginBottom:10}}>I tuoi compiti di oggi</div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {tasks.map(task=>(
              <button key={task.id} onClick={()=>completeTask(task.id)} style={{
                ...
                border:`2px solid ${task.done?C.mintDark:C.mintLight}`,borderRadius:18,padding:"13px 16px",
                display:"flex",alignItems:"center",gap:12,cursor:task.done?"default":"pointer",
                boxShadow:task.done?"none":"0 3px 14px rgba(0,0,0,0.07)",transition:"all 0.3s",
                textAlign:"left",width:"100%"}}>
                <div style={{width:42,height:42,borderRadius:"50%",flexShrink:0,
                  background:task.done?C.mintDark:C.mintLight,display:"flex",alignItems:"center",
                  justifyContent:"center",fontSize:task.done?20:22,transition:"all 0.3s",color:task.done?"white":"inherit"}}>
                  {task.done?"✓":task.icon}
                </div>
                <div style={{flex:1}}>
                  <div style={{fontWeight:800,fontSize:15,color:task.done?C.textMed:C.textDark,
                    textDecoration:task.done?"line-through":"none"}}>{task.name}</div>
                  {task.done&&task.doneTime&&<div style={{fontSize:11,color:C.mintDeep,fontWeight:700,marginTop:2}}>✓ fatto alle {task.doneTime}</div>}
                  {task.essential&&!task.done&&<div style={{fontSize:10,color:C.pinkDeep,fontWeight:800,marginTop:2}}>⚡ fondamentale</div>}
                </div>
                {!task.done&&<div style={{background:`linear-gradient(135deg,${C.mint},${C.mintDark})`,borderRadius:10,padding:"5px 12px",fontSize:12,fontWeight:900,color:"white"}}>Fatto!</div>}
              </button>
            ))}
          </div>
          ...
          <div style={{textAlign:"center",marginTop:28,paddingBottom:24}}>
            <button onClick={()=>setEmergencyOpen(!emergencyOpen)} style={{background:"transparent",
              border:`1px solid ${C.pink}`,color:C.textLight,fontSize:12,fontWeight:700,
              padding:"6px 18px",borderRadius:20,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>📞 parla con la mamma</button>
            {emergencyOpen&&(
              <div style={{marginTop:10,background:C.white,borderRadius:16,padding:14,border:`1.5px solid ${C.pink}`,textAlign:"left"}}>
                <div style={{display:"flex",gap:8}}>
                  <input value={emergencyMsg} onChange={e=>setEmergencyMsg(e.target.value)}
                    placeholder="Scrivi alla mamma..."
                    style={{flex:1,padding:"9px 13px",borderRadius:11,border:`1.5px solid ${C.pink}`,outline:"none",fontFamily:"'Nunito',sans-serif",fontSize:13}}/>
                  <button style={{padding:"9px 14px",borderRadius:11,border:"none",background:C.pinkDark,color:"white",fontWeight:900,cursor:"pointer"}}>→</button>
                </div>
              </div>
            )}
          </div>
        ...
        {/* MOM VIEW */}
        {view==="mom"&&(<>
          <div style={{background:vacation?`linear-gradient(135deg,${C.yellow},#FFD93D)`:C.white,borderRadius:20,
            padding:"16px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",
            marginBottom:16,boxShadow:"0 4px 20px rgba(0,0,0,0.08)",transition:"background 0.4s"}}>
            <div>
          ...
          <div style={{background:C.white,borderRadius:20,padding:"16px 20px",marginBottom:16,boxShadow:"0 4px 20px rgba(0,0,0,0.08)"}}>
            ...
            <div style={{marginTop:12,padding:"10px 14px",background:(!vacation&&!essentialsDone)?"#FFF0F0":C.mintLight,
              borderRadius:12,fontWeight:800,fontSize:13,color:(!vacation&&!essentialsDone)?C.red:C.mintDeep}}>
          ...
          <div style={{background:C.white,borderRadius:20,padding:"16px 20px",boxShadow:"0 4px 20px rgba(0,0,0,0.08)"}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:14}}>
              <input value={newTask} onChange={e=>setNewTask(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addTask()}
                placeholder="Aggiungi compito..." style={{flex:1,padding:"10px 14px",borderRadius:12,
                  border:`2px solid ${C.mintLight}`,outline:"none",fontFamily:"'Nunito',sans-serif",fontSize:14,color:C.textDark}}/>
